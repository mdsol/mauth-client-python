...
class TestAuthenticator(unittest.TestCase):
    ...
    def test_is_authentic_mauth_not_present(self):
        ...
        authentic, status, message = self.mock_authenticator.is_authentic()
        expected = "Authentication Failed. No mAuth signature present; " \
        ...
        self.assertEqual(self.captor.getvalue(),
                         "mAuth signature not present on request. Exception: {}\n".format(expected))

    def test_is_authentic_v2_only_with_v1_headers(self):
        self.logger.setLevel(logging.ERROR)
        self.mock_authenticator = MockAuthenticator(self.v1_headers, True)
        authentic, status, message = self.mock_authenticator.is_authentic()
        expected = "This service requires mAuth v2 mcc-authentication header " \
